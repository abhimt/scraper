...
def summarize_alt(courses):
    ...
    for course in tqdm(courses):
        inputs = tokenizer(course[3], max_length=1024, return_tensors='pt',
                        truncation=True, padding=True)
        # this change with the models.
        outputs = model.generate(inputs['input_ids'], num_beams=4,
                                max_length=1024, early_stopping=True)
        ...
        add_sort_des(course[2],course[0], summary[0])
        # return summary[0]
    # for i in range(len(courses)):
    #     courses[i][3] = summaries[i]
    # return courses
...
def extract_lists(university):
    # programs = select_programs(university)
    # for program in programs:
        # print(f'Extracting courses for {program[0]}')
    courses = select_programs_courses(university)
    print(f'Total numbers of courses: {len(courses)}')
    # for course in courses:
        # yield course
        # print(f'Extracting descriptions for {course[0]}\n')
    summarize_alt(courses)
...
if __name__ == '__main__':
    ...
    if args.test:
        ...
        courses, descriptions = select_courses(
            program, university_id, arguments['courses'])
        print(descriptions)
        print('ok\n')
        data = summarize(courses, descriptions)
        if args.print:
